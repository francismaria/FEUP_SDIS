...
public class MDBchannel extends ChannelInformation implements Runnable{
	...
	public void run() {
		...
		System.out.println("PEER " + getPeer().getId() + ": " + "Connection Established -> MDB CHANNEL");
	...
	public void parsePUTCHUNKMessage(byte[] message, int messageLength) {
		...
		System.out.println("PEER " + getPeer().getId() + ": RECEIVED " + receivedMessage.getType() + " FROM PEER " + receivedMessage.getSenderId() + " | CHUNK NO: " + receivedMessage.getChunkNo());
